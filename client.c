...
static int execute_requestreply(int connfd, operation_t *o) {					//passo il descrittore connessione e la struct 
    char *sname = o->sname;
    ...
    if (op == POSTTXT_OP || op == POSTTXTALL_OP || op == POSTFILE_OP) {		//se devo spedire il file
	if (o->size == 0) {
...
int main(int argc, char *argv[]) {
    ...
    operation_t *ops = NULL;				//inizializzo la struct
    long msleep=0;
    ...
    ops = (operation_t*)malloc(sizeof(operation_t)*(argc-3));			//alloco la struct
    if (!ops) {
	perror("malloc");
	return -1;
    }
